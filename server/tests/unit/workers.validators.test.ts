...
describe("workerProfileSchema", () => {
  ...
  it("rejects short headline", () => {
    expect(
      workerProfileSchema.safeParse({ ...valid, headline: "Hi" }).success,
    ).toBe(false);
  });

  it("rejects bio under 10 chars", () => {
    expect(
      workerProfileSchema.safeParse({ ...valid, bio: "short" }).success,
    ).toBe(false);
  });

  it("rejects bio over 2000 chars", () => {
    expect(
      workerProfileSchema.safeParse({ ...valid, bio: "a".repeat(2001) })
        .success,
    ).toBe(false);
  });
  ...
  it("rejects negative experience", () => {
    expect(
      workerProfileSchema.safeParse({ ...valid, yearsExperience: -1 }).success,
    ).toBe(false);
  });

  it("rejects experience over 70", () => {
    expect(
      workerProfileSchema.safeParse({ ...valid, yearsExperience: 71 }).success,
    ).toBe(false);
  });

  it("rejects negative hourly rate", () => {
    expect(
      workerProfileSchema.safeParse({ ...valid, hourlyRate: -1 }).success,
    ).toBe(false);
  });

  it("rejects radius > 100km", () => {
    expect(
      workerProfileSchema.safeParse({ ...valid, serviceRadiusKm: 101 }).success,
    ).toBe(false);
  });

  it("rejects radius < 1km", () => {
    expect(
      workerProfileSchema.safeParse({ ...valid, serviceRadiusKm: 0 }).success,
    ).toBe(false);
  });

  it("accepts null hourlyRate", () => {
    expect(
      workerProfileSchema.safeParse({ ...valid, hourlyRate: null }).success,
    ).toBe(true);
  });
});
...
describe("workerProfileUpdateSchema", () => {
  it("accepts partial updates", () => {
    expect(
      workerProfileUpdateSchema.safeParse({ headline: "New headline here" })
        .success,
    ).toBe(true);
  });
  it("rejects unknown fields", () => {
    expect(workerProfileUpdateSchema.safeParse({ foo: "bar" }).success).toBe(
      false,
    );
  });
...
describe("upsertSkillsSchema", () => {
  ...
  it("rejects non-uuid skillId", () => {
    expect(
      upsertSkillsSchema.safeParse({ skills: [{ skillId: "not-uuid" }] })
        .success,
    ).toBe(false);
  });
  it("rejects invalid level", () => {
    expect(
      upsertSkillsSchema.safeParse({
        skills: [
          { skillId: "11111111-1111-1111-1111-111111111111", level: "GOD" },
        ],
      }).success,
    ).toBe(false);
  });
...
describe("portfolioCreateSchema", () => {
  ...
  it("rejects non-url", () => {
    expect(
      portfolioCreateSchema.safeParse({ imageUrl: "not-a-url" }).success,
    ).toBe(false);
  });
...
describe("workerListQuerySchema", () => {
  it("coerces numeric and boolean strings", () => {
    const r = workerListQuerySchema.safeParse({
      page: "2",
      pageSize: "50",
      minRating: "4.0",
      verifiedOnly: "true",
  ...
  it("rejects bad sort", () => {
    expect(workerListQuerySchema.safeParse({ sort: "price:asc" }).success).toBe(
      false,
    );
  });
});
...
describe("adminVerifyWorkerSchema", () => {
  it("requires isVerified", () => {
    ...
    expect(
      adminVerifyWorkerSchema.safeParse({ isVerified: true }).success,
    ).toBe(true);
  });
});
