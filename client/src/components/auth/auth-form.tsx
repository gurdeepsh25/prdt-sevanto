...
/**
 * Generic form wrapper. Renders an inline error banner on ApiError or any thrown error,
 * and surfaces per-field errors via the `fieldErrors` prop.
 */
export function AuthForm({
  ...
  return (
    <form onSubmit={handle} className={cn("space-y-4", className)} noValidate>
      ...
      {children({ fieldErrors, setFieldErrors })}
      <button
        type="submit"
        disabled={loading}
